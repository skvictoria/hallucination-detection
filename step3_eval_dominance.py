...
def sync_image_token_index(model, tokenizer) -> int:
    """
    tokenizer의 "<image>" 토큰 id와 model.config.image_token_index가 어긋나면
    generate/forward에서 image token count 계산이 틀어질 수 있어서 동기화.
    """
...
def ensure_safe_pad(tokenizer, model):
    """
    pad_token_id가 image_token_index와 같아지면 padding이 곧 image token으로 카운트되어
    "image tokens K vs images 1" 에러가 발생.
    """
    if tokenizer.pad_token is None:
        tokenizer.add_special_tokens({"pad_token": "<pad>"})
        model.resize_token_embeddings(len(tokenizer))

    # image token id를 tokenizer 기준으로 먼저 동기화
    sync_image_token_index(model, tokenizer)

    if tokenizer.pad_token_id == model.config.image_token_index:
        # 충돌이면 pad 토큰을 하나 더 만들어 분리
...
def build_prompt(question: str, choices: Optional[List[str]]) -> str:
    if choices and isinstance(choices, list) and len(choices) > 0:
        opts = " ".join([f"({chr(65+i)}) {c}" for i, c in enumerate(choices[:5])])
...
def prepare_llava_inputs_no_padding(
    ...
    """
    AutoProcessor를 쓰지 않고, tokenizer + image_processor를 직접 사용.
    padding=False 강제해서 pad/image token 충돌로 인한 image-token-count 폭증을 제거.
    """
    ...
    if cnt != 1:
        if debug:
            ...
            # (attention_mask==0) 위치가 있으면 pad로 추정
...
# -------------------------
# Generation wrapper (NO-PADDING)  <-- critical fix
# -------------------------
@torch.no_grad()
def generate_answer(
    model,
